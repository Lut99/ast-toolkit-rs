...
fn emit_diagnostic_header(
    ...
    writeln!(
        writer,
        "{}{}{}",
        accent_colour.apply_to(kind),
        if let Some(code) = code { accent_colour.apply_to(format!("[{code}]")) } else { accent_colour.apply_to(format!("")) },
        style(format!(": {message}")).bold()
    )
}
...
/// This function may error if we failed to write on the given writer.
#[inline]
fn emit_diagnostic_source_lines<'f, 's>(
    writer: &mut impl Write,
    accent_colour: &Style,
    source: Span<'f, 's>,
    note: Option<&str>,
...
pub enum DiagnosticKind {
    ...
    ///
    /// assert_eq!(
    ///     Diagnostic::error("Example", Span::new("<example>", "example")).kind(),
...
///
/// let err =
///     Diagnostic::error("Invalid word 'Hlelo'", Span::ranged("<example>", "Hlelo World!", 0..=4));
/// let warn = Diagnostic::warn(
///     "Second word shouldn't be capitalized",
///     Span::ranged("<example>", "Hlelo World!", 6..=6),
/// );
/// let note = Diagnostic::note(
///     "The most classical program in the world is given here",
///     Span::new("<example>", "Hlelo World!"),
/// );
/// let suggest = Diagnostic::suggestion(
///     "Consider writing it properly",
///     Span::new("<example>", "Hlelo World!"),
///     "Hello, world!",
/// );
/// ```
...
impl<'f, 's> Diagnostic<'f, 's> {
    ...
    pub fn error(message: impl Into<String>, span: impl IntoSpan<'f, 's>) -> Self {
        Self { message: message.into(), code: None, remark: None, span: span.into_span(), kind: DiagnosticSpecific::Error, sub: vec![] }
    }
    ...
    ///
    /// let diag: Diagnostic =
    ///     Diagnostic::warn("An example warning.", Span::new("<example>", "Example"));
    /// ```
    #[inline]
    ...
    pub fn note(message: impl Into<String>, span: impl IntoSpan<'f, 's>) -> Self {
        Self { message: message.into(), code: None, remark: None, span: span.into_span(), kind: DiagnosticSpecific::Note, sub: vec![] }
    }
    ...
    /// use ast_toolkit::{Diagnostic, Span};
    ///
    /// let diag: Diagnostic = Diagnostic::suggestion(
    ///     "An example suggestion.",
    ///     Span::new("<example>", "Example"),
    ///     "A better example.",
    /// );
    /// ```
    #[inline]
    ...
    /// use ast_toolkit::{Diagnostic, Span};
    ///
    /// Diagnostic::error("An example error.", Span::new("<example>", "Example")).set_code("1").emit();
    /// ```
    #[inline]
    ...
    /// let span = Span::ranged("<example>", "Hello, World!", 7..=11);
    /// assert_eq!(
    ///     Diagnostic::suggestion("Try writing 'World' lowercase", span, "world").replacement(),
    ///     "world"
    /// );
    /// ```
    #[inline]
    ...
    pub fn replacement(&self) -> &str {
        if let DiagnosticSpecific::Suggestion { replace } = &self.kind {
            replace
        } else {
            panic!("Cannot return the code of a non-Suggestion Diagnostic (is {})", self.kind.variant());
        }
    }
    ...
    fn _emit_on(&self, writer: &mut impl Write, toplevel: bool) -> Result<(), std::io::Error> {
        ...
        if !self.span.is_empty() {
            ...
            let (source, accent_range): (Cow<str>, (Option<usize>, Option<usize>)) = match &self.kind {
                ...
                DiagnosticSpecific::Error | DiagnosticSpecific::Warning | DiagnosticSpecific::Note => {
                    (Cow::Borrowed(self.span.source), self.span.range)
                },
            };
            ...
            emit_diagnostic_source_lines(
                writer,
                &accent_colour,
                Span { file: self.span.file, source: source.as_ref(), range: accent_range },
                self.remark.as_ref().map(|r| r.as_str()),
            )?;
        }
    ...
    #[inline]
    #[track_caller]
    pub fn emit_on(&self, writer: &mut impl Write) -> Result<(), std::io::Error> { self._emit_on(writer, true) }
    ...
    #[inline]
    #[track_caller]
    pub fn emit(&self) { self.emit_on(&mut std::io::stderr()).unwrap(); }
    ...
    #[inline]
    #[track_caller]
    pub fn abort(&self) -> ! { self.abort_on(&mut std::io::stderr()).unwrap(); }
}
