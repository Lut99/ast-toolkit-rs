...
use crate::error_new::expects_tag;
...
impl<F, S, T: Debug> Expects for Tag<F, S, T> {
    #[inline]
    fn fmt(&self, f: &mut Formatter, _indent: usize) -> FResult { expects_tag(f, self.tag) }
}
