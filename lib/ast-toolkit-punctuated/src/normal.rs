...
//  Last edited:
//    26 Feb 2024, 15:52:17
//  Auto updated?
//    Yes
//
//  Description:
//!   Implements the "normal" punctuated list, which has mandatory punctuation
...
impl<V, P> Punctuated<V, P> {
    /// Constructor for the Punctuated that initializes it as an empty list.
    ///
    /// # Returns
    ...
    ///
    /// # Returns
    /// [`NextValue::Value`] if a value should be given next (i.e., [`Self::accepts_value()`](Punctuated::accepts_value()) is true);  
    /// or [`NextValue::Punctuation`] if a punctuation should be given next (i.e., [`Self::accepts_punct()`](Punctuated::accepts_punct()) is true)
    ///
    /// # Example
    /// ```rust
    /// use ast_toolkit_punctuated::{NextValue, Punctuated};
    ///
    /// let mut list = Punctuated::<i32, char>::new();
