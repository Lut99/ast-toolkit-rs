...
impl<F, S> Diagnostic<F, S> {
    ...
    #[inline]
    pub fn new(kind: DiagnosticKind, message: impl ToString, span: impl Into<Span<F, S>>) -> Self {
        Self {
            kind,
    ...
    /// A new instance of Self.
    #[inline]
    pub fn note(message: impl ToString, span: impl Into<Span<F, S>>) -> Self {
        Self {
    ...
    /// A new instance of Self.
    #[inline]
    pub fn suggestion(message: impl ToString, span: impl Into<Span<F, S>>) -> Self {
        Self {
    ...
    /// A new instance of Self.
    #[inline]
    pub fn warn(message: impl ToString, span: impl Into<Span<F, S>>) -> Self {
        Self {
    ...
    /// A new instance of Self.
    #[inline]
    pub fn error(message: impl ToString, span: impl Into<Span<F, S>>) -> Self {
        Self {
    ...
    #[inline]
    pub fn emit_on(&self, mut writer: impl Write) -> Result<(), std::io::Error> {
        // Fetch the source lines that we'll want to write.
        let lines: Vec<&str> = self.span.lines();

        // Resolve the style for this note
        let (kind, colour): (&'static str, Style) = match self.kind {
            DiagnosticKind::Error      => ("error", Style::new().bold().red()),
            DiagnosticKind::Warning    => ("warning", Style::new().bold().yellow()),
            DiagnosticKind::Note       => ("note", Style::new().bold().green()),
            DiagnosticKind::Suggestion => ("suggestion", Style::new().bold().cyan()),
        };

        // Write the header
        writeln!(writer, "{}{}{}", colour.apply_to(kind), style(": ").bold(), style(&self.message).bold())?;

        // Write the file indicator
        writeln!(writer, "  {} {}:{}", style("-->").blue(), )?;
